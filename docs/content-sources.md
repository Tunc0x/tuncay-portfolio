...
- Fraunhofer CML reference letter, DIOR IT contribution: worked with the Unity WebRTC package to get the video stream working in the VR application and added a laser marker projected onto the 360° video sphere. This does not imply ownership of the overall streaming system or architecture. The Spot photograph shows the physical system.
- Other Fraunhofer CML work, separate from DIOR IT: reverse-Panini projection shader, analysis and correction of marker displacement caused by projection offsets, and interactive panels supporting scaling, zooming and movement.
